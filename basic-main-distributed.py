...
def main(args):
    ...
    if args.url == "env://" and args.world_size == -1:
        args.world_size = int(os.environ["WORLD_SIZE"])
    if args.url == "env://" and args.local_rank == -1:
    ...
    network = base_model.to(device)
    network = torch.nn.SyncBatchNorm.convert_sync_batchnorm(network)
    network = torch.nn.parallel.DistributedDataParallel(
        network, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True)
    ...
    if last_info.exists():  # automatically resume from previous checkpoint
        ...
        last_infox = torch.load(last_info, map_location=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/data/zhy_data/cifar", help="The dataset name.")
    ...
    parser.add_argument("--number", type=str, help="experiment number")
    # Acceleration
    ...
    parser.add_argument('--url', default='env://', type=str, help='url used to set up distributed training')

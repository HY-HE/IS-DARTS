...
class NAS201SearchCell(nn.Module):
    ...
    def forward_isdarts(self, inputs, masks):
        nodes = [inputs]
        for i in range(1, self.max_nodes):
            inter_nodes = []
            for j in range(i):
                node_str = "{:}<-{:}".format(i, j)
                mask = masks[self.edge2index[node_str]]
                inter_nodes.append(
                    # sum(layer(nodes[j]) * m / torch.sum(mask)
                    sum(layer(nodes[j]) * m / self.num_ops
                        for layer, m in zip(self.edges[node_str], mask))
                )
            nodes.append(sum(inter_nodes))
        return nodes[-1]
...
class MixedOp(nn.Module):
    ...
    def forward_isdarts(self, x, mask, num_ops):
        return sum(m * op(x) / num_ops for m, op in zip(mask, self.ops))
...
class NASNetSearchCell(nn.Module):
    def __init__(
        ...
        if reduction_prev:
            ...
            # actually Factorized Reduce, if prev prev cell outputs 4C*H*W, curr cell outputs 2C*H/2*W/2
        else:
    ...
    def forward_isdarts(self, s0, s1, masks):
        s0 = self.preprocess0(s0)
        s1 = self.preprocess1(s1)
        ...
        for i in range(self._steps):
            ...
            for j, h in enumerate(states):
                ...
                op = self.edges[node_str]
                mask = masks[self.edge2index[node_str]]
                clist.append(op.forward_isdarts(h, mask, self.num_ops))
            states.append(sum(clist))
        return torch.cat(states[-self._multiplier:], dim=1)

...
class NASNetworkISDartsImageNet(nn.Module):
    def __init__(self, C: int, N: int, steps: int, multiplier: int, stem_multiplier: int, num_classes: int,
        ...
        super(NASNetworkISDartsImageNet, self).__init__()
        self._C = C
        self._layerN = N
        self._steps = steps
        ...
        C_prev_prev, C_prev, C_curr, reduction_prev = C * stem_multiplier, C * stem_multiplier, C, True
    ...
    def get_weights(self) -> List[torch.nn.Parameter]:
        xlist = list(self.stem0.parameters()) + list(self.stem1.parameters()) + list(self.cells.parameters())
            ...
            for i in range(self._steps):
                ...
                for j in range(2 + i):
                    ...
                    for k, op_name in enumerate(self.op_names):
                        ops.append((op_name, i, j, k, masks[self.edge2index[node_str]][k]))

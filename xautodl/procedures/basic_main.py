...
def basic_valid(
    xloader, device, network, criterion, optim_config, extra_info, print_freq, logger
):
    with torch.no_grad():
        loss, acc1, acc5 = procedure(
            xloader,
...
def procedure(
    ...
    for i, (inputs, targets) in enumerate(xloader):
        ...
        targets = targets.cuda(device, non_blocking=True)  # modified: also puts targets on cuda
